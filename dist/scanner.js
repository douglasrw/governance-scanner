...
export async function scanRepo(repoUrl) {
    ...
    if (hasGovernanceConfig) {
        ...
        findings.push({
            ...
            description: "Structural AI guidance found in CLAUDE.md, .claude/CLAUDE.md, Claude settings surfaces such as .claude/settings.json and .claude/settings.local.json, GEMINI.md, .gemini/GEMINI.md, AGENTS.md, .claude, .cursorrules, .github/copilot-instructions.md, .cursor/rules, data/agents, data/roles, scripts/agents, or .github/instructions/*.instructions.md.",
        });
    }
    else {
        findings.push({
