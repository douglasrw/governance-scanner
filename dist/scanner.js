...
export async function scanRepo(repoUrl) {
    ...
    if (hasTestConfig)
        testScore += 5;
    if (hasTestDir || hasScatteredTestFile)
        testScore += 5;
    if (testScore > 0) {
        findings.push({
            severity: "positive",
            title: "Testing infrastructure",
            description: "Test configuration or conventional test files/directories are present.",
        });
    }
    else {
        findings.push({
            ...
            description: "No test config, test directories, or conventional test files found.",
        });
    }
    dimensions.push({
        name: "Testing",
    ...
    if (hasGovernanceConfig) {
        ...
        findings.push({
            ...
            description: "Structural AI guidance found in CLAUDE.md, claude.md, .claude/CLAUDE.md, Claude settings surfaces such as .claude/settings.json and .claude/settings.local.json, .claude/commands/* guidance surfaces, GEMINI.md, gemini.md, .gemini/GEMINI.md, AGENTS.md, agents.md, .claude, .cursorrules, .github/copilot-instructions.md, .cursor/rules, data/agents, data/roles, scripts/agents, or .github/instructions/**/*.instructions.md.",
        });
    }
    else {
        findings.push({
